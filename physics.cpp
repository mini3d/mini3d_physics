...
void doGJK(const Transform &tfa, const Transform &tfb, const Hull &hull, const Hull &hull2, Simplex &s, Vec3 &axis) {
    ...
    for ( ;; ) {
        ...
        float distance_dot_axis = axis.Dot(-s[0].p);
        float distanceToSimplex = distance_dot_axis * distance_dot_axis / axis.Norm();
...
void preSolvePosition(Manifold &manifold) {
...
void solvePosition(Manifold &manifold) {
    ...
    for (int i = 0; i < manifold.contactsCount; ++i) {
        ...
        manifold.body[FIRST]->transform.rot = 0.5f * Ternion(-clampedCorrection * contact.inertia[FIRST][n]) * manifold.body[FIRST]->transform.rot;
        manifold.body[SECOND]->transform.rot = 0.5f * Ternion(clampedCorrection * contact.inertia[SECOND][n]) * manifold.body[SECOND]->transform.rot;
    }
}
...
template <class T> void findIsland(Body* body, T callback) {
    ...
    if (body->isPartOfIsland || body->isFixed()) { // TODO: Dont look at sleeping bodies when putting islands to sleep
        return;
    }
...
void Physics::step(float timeStep, const Physics::CollisionCallbacks &callback) {
    ...
    if (!paused) {
        for (int i = 0; i < bodies.size(); ++i) {
            if (bodies[i].isAwake && !bodies[i].isFixed()) {
                ...
                bodies[i].angularVelocity *= 0.999f;
            }
        }
    ...
    for (auto &it : manifolds) {
        if (!it.ignoreContacts) {
            doContacts(it);
        }
        
        // Call the callbacks
        if (it.contactsCount > 0 && !it.contactStartedCalled) {
            it.ignoreContacts = !callback.contactStarted(it.body[FIRST], it.body[SECOND]);
            if (it.ignoreContacts) {
                it.contactsCount = 0;
            }
            it.contactStartedCalled = true;
        } else if (it.contactsCount == 0 && it.contactStartedCalled) {
            callback.contactEnded(it.body[FIRST], it.body[SECOND]);
            it.contactStartedCalled = false;
            it.ignoreContacts = false;
        }
    }
    ...
    if (!paused) {
        ...
        // Do shock propagation

        // find all bodies with a connection to a fixed body and set them to shockLevel 0
        for (int i = 0; i < bodies.size(); ++i) {
            Body &body = bodies[i];
            if (body.isFixed()) {
                body.shockLevel = 0;
            } else {
                body.shockLevel = SIZE_T_MAX;
            }
        }
        ...
        if (ENABLE_SLEEPING) {
            for(auto &body : bodies) {
                body.isPartOfIsland = false;
                body.sleepTimer = (body.velocity.Norm() < 0.0005f && body.angularVelocity.Norm() < 0.000001f) ? body.sleepTimer + timeStep : 0;
            }
